...
var windowWidth = 1280, windowHeight=1080;
...
function setup() {
	...
	prevsnap = createImage(windowWidth, windowHeight,RGB);
}
...
function draw() {
		...
		for (var x=0; x < snap.height; x++ ){
			...
			*/
			...
			if (similarity > threshold){
				pixels[index] = random(1,255);
				pixels[index+1] = random(1,255);
				pixels[index+2] = random(1,255);
				pixels[index+3] = 255;
			}
	...
	updatePixels();
	prevsnap.copy(snap, 0, 0, windowWidth, windowHeight, 0, 0, windowWidth, windowHeight);
}